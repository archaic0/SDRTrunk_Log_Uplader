...
It will cache JSON files if the upload fails.  It tracks which log file was last processed and which event_id was last processed and will catch up from where it left off.  The first run will process the entire log history in the log directory, so you might start with empty logs unless a large initial back-post is desired.
